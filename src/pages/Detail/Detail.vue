        ...
        <div class="InfoWrap">
          ...
          <div class="choose">
            ...
            <div class="cartWrap">
              ...
              <div class="add">
                <!-- 加入购物车，进行路由跳转之前，发请求把购买的产品的信息通过请求的形式通知服务器，服务器进行相应的存储 -->
...
export default {
  ...
  methods: {
    ...
    changeActive(saleAttrValue, arr) {
      // console.log(arr)
      // 遍历全部售卖属性值isChecked为0（所有选中的高亮都没有）
      arr.forEach((item) => {
        item.isChecked = 0;
      });
      // 点击的那个售卖属性值要有高亮
    ...
    async addShopCar() {
      // 发请求---将产品加入到数据库（通知服务器）
      /*当前这里是派发一个action，也向服务器发请求,但是还需要判断加入购物车是成功还是失败了,进行相应的操作
       this.$store.dispatch('addUpdateShopCart',{skuId:this.$route.params.skuid, skuNum:this.skuNum })
       上面这行代码：就是在调用addUpdateShopCart（）函数
       addUpdateShopCart（）函数最后会返回一个promise（要么成功要么失败）
       */
      try {
        ...
        // 路由跳转
        // 在路由跳转的时候还需要将产品的信息带给addcartsuccess路由组件
        // 一些简单的数据skuNum,通过query形式给路由组件传递过去
        // 产品信息的数据比较复杂（通过回话存储：会话结束数据就会消失）
        // 本地存储和会话存储一般存储的是字符串
